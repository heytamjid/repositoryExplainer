...
def build_repo_map(
    ...
    """On-demand build of repository map only (no embeddings).

    This is used when a repository was indexed before map support was added or
    the map file was removed. It clones the repository, walks files with the
    same skip logic as indexing, extracts lightweight unit spans and persists
    the map JSON. Returns the map dict on success, or None on failure.
    """
    print(f"[REPO_MAP] Building repository map on-demand for {repo_url} ...")
    tmpdir = Path(tempfile.mkdtemp(prefix="repo_map_build_"))
    try:
        commit = _clone_repo(repo_url, tmpdir)
        if not commit:
            print("[REPO_MAP] Failed to clone repo for map generation")
            return None
        spec = _read_gitignore(tmpdir)
        repo_map_files: List[Dict] = []
        for root, _, files in os.walk(tmpdir):
        ...
        if not repo_map_files:
            print("[REPO_MAP] No files discovered while building map")
            return None
        try:
            _write_repo_map(repo_url, commit, repo_map_files, persist_dir=persist_dir)
            print(
                f"[REPO_MAP] Repository map built with {len(repo_map_files)} files; saved to {_repo_map_path(repo_url, persist_dir)}"
            )
        except Exception as e:
            print(f"[REPO_MAP] Failed to persist repository map: {e}")
            return None
        return get_repo_map(repo_url, persist_dir)
    finally:
        try:
            shutil.rmtree(tmpdir, ignore_errors=True)
        except Exception:
